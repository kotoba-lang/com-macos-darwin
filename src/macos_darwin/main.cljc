(ns macos-darwin.main
  "Kotodama WASM entrypoint for the macOS Darwin clean-room actor (L4) — Clojure port.
  ...
  facts (`macos_darwin.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "macos_darwin")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"macos_darwin.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "macos_darwin-compat" :tier tier :entities entities} 200])
...
(defn start! [] :macos_darwin-compat/ready)
